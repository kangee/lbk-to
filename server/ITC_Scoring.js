Meteor.ITC = {
reportResult : function(tournament, round , table , playerOne , playerOneScore , playerTwo , playerTwoScore){
		var currentPlayerId = null;
		if(Meteor.validationHelpers.isTo(tournament) || Meteor.validationHelpers.isPlayer(tournament, playerOne) || Meteor.validationHelpers.isPlayer(tournament, playerTwo) ){
			...
			for (var i = _players.length - 1; i >= 0; i--) {
				if (_players[i].Id === playerOne){
					...
					if(Number(playerOneScore) > Number(playerTwoScore)){
						_players[i].Points = Number(_players[i].Points) + 1000;
					}else if(Number(playerOneScore) === Number(playerTwoScore)){
						_players[i].Points = Number(_players[i].Points) + 500;
					}
				}
	...
	updateResult : function(tournament,round , table , playerOne , playerOneScore , playerTwo , playerTwoScore){
		...
		for (var i = _players.length - 1; i >= 0; i--) {
			if (_players[i].Id === playerOne){
				...
				if(Number(playerOneOld) > Number(playerTwoOld)){
					_players[i].Points = Number(_players[i].Points) - 1000;
				}else if(Number(playerOneOld) === Number(playerTwoOld)){
					_players[i].Points = Number(_players[i].Points) - 500;
				}
				if(Number(playerOneScore) > Number(playerTwoScore)){
					_players[i].Points = Number(_players[i].Points) + 1000;
	...
	return function(player1, player2){
		...
		//opponents score
		playerOneOpponents = 0;
		playerTwoOpponents = 0;
		for (var i = 0; i < tournament.Players.length; i++) {
			if(Meteor.Sorter.IndexOf(tournament.Players[i].Oponents, player1.Id) != -1){
				playerOneOpponents = playerOneOpponents + tournament.Players[i].Points;
			}
			if(Meteor.Sorter.IndexOf(tournament.Players[i].Oponents, player2.Id) != -1){
				playerTwoOpponents = playerTwoOpponents + tournament.Players[i].Points;
			}
		}
		if(playerOneOpponents != playerTwoOpponents){
			console.log("Sorted on Opponents Score:" + player1.Name + "-" + player2.Name);
			return playerTwoOpponents - playerOneOpponents;
		}
